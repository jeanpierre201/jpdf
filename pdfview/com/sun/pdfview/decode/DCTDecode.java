...
public class DCTDecode {
	...
	//	System.out.println("DCTDecode image info: "+params);
        buf.rewind();
       // Log.d("(Seb) ANDPDF.dctdecode", "params "+params);
    	...
    	Log.d("(Seb) ANDPDF.dctdecode", "decoded image type "+conf);
    	
    	int size = 4*img.getWidth()*img.getHeight();				// Config.ARGB_8888 seb
    	if (conf == Config.RGB_565) 
    		size = 2*img.getWidth()*img.getHeight();
    	
    	
    	// TODO [FHe]: else ... what do we get for gray? Config.ALPHA_8?
    	// Log.d("(Seb) ANDPDF.dctdecode", "decoded image size "+size);
        ...
        Log.d("(Seb) ANDPDF.dctdecode", "decoded image done.");
