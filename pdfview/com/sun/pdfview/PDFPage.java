...
public class PDFPage {
    ...
    private final Map<ImageInfo, PDFRenderer> renderers = Collections.synchronizedMap(new HashMap<ImageInfo, PDFRenderer>());
...
//            }
 ...
 // Created By JP Thumbnails......   
    ...
    public Bitmap getImageT(int width, int height, RectF clip,
            boolean drawbg, boolean wait) {
        // see if we already have this image
        Bitmap image = null;
        PDFRenderer renderer = null;
...
//        mat.setTranslate(-clip.top, -clip.left);
    ...
    public void stop(int width, int height, RectF clip) {
        ...
        synchronized (renderers) {
            // find our renderer
        	PDFRenderer rendererRef = renderers.get(info);
            if (rendererRef != null) {
                // PDFRenderer renderer = (PDFRenderer) rendererRef.get();
            	PDFRenderer renderer = rendererRef;
                if (renderer != null) {
                    // stop it
                    renderer.stop();
                }
...
//        }
    ...
    public void updateImages() {
        ...
        for (Iterator i = renderers.values().iterator(); i.hasNext();) {
        	...
        	PDFRenderer ref = (PDFRenderer) i.next();
            // PDFRenderer renderer = (PDFRenderer) ref.get();
        	PDFRenderer renderer = (PDFRenderer) ref;
